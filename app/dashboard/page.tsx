...
import DiabetesPanel from './diabetes-panel';
import RetiZeroPanel from './retizero-panel';
...
export default function Home() {
 ...
 async function upload(file?: File) {
  ...
  try {
   ...
   selectCase(id); setModal(false); setNotice('Image opened locally. Image-quality signals are ready; Use the RetiZero panel to connect image prediction.');
 ...
 return <div className="shell">
  ...
  <aside className="sidebar">
   ...
   <nav aria-label="Main navigation">{[['Workspace', LayoutGrid], ['Evaluation', Activity], ['Diabetes API', Activity], ['Project', FlaskConical]].map(([name, Icon]) => <button key={String(name)} className={tab === name ? 'nav-item active' : 'nav-item'} onClick={() => setTab(String(name))}>{typeof Icon !== 'string' && <Icon size={18}/>}<span>{String(name)}</span>{name === 'Workspace' && <span className="nav-count">{cases.length}</span>}</button>)}</nav>
  ...
  <main id="main">
   ...
   {tab === 'Diabetes API' ? <DiabetesPanel/> : tab === 'Workspace' ? <>
   ...
   <RetiZeroPanel key={current.id} image={current.image} hash={current.imageSha256}/>
